...
- 認証: ユーザー名/パスワードによるセッション(Cookie)認証。複数アカウントで個別にセーブ/ロード（Spring Security + BCrypt）
...
- **ゲーム内リセット（開発用）**: `POST /api/reset` を叩くとログイン中ユーザーのセーブを消去する。
  （オンボーディングからやり直せる）
...
ユーザー名/パスワードのセッション(Cookie)認証（Spring Security）。パスワードはBCryptでハッシュ化して保存し、
平文は保存しない。`/api/auth/**` 以外の全APIはログイン必須で、未ログインは 401 を返す。
セーブデータ(`PlayerState`/`OwnedCharacter`)はログイン中のユーザー名で紐付き、アカウントごとに分離される。

| メソッド | パス | 説明 |
|---------|------|------|
| POST | `/api/auth/register` | 新規登録 `{username, password}`（成功時は自動ログイン） |
| POST | `/api/auth/login` | ログイン `{username, password}` |
| POST | `/api/auth/logout` | ログアウト(セッション破棄) |
| GET  | `/api/auth/me` | ログイン中ユーザーの取得(未ログインは401) |
...
- アカウント削除・パスワード再設定など運用系機能は未実装（登録・ログイン・ログアウトのみ）。
