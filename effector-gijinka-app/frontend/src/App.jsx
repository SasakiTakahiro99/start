...
export default function App() {
  const [user, setUser] = useState(undefined) // undefined=確認中, null=未ログイン, {username}=ログイン済み
  ...
  if (user === undefined || booting) return <div className="center">読み込み中…</div>

  if (!user) {
    return (
      <>
        <AuthScreen onLoggedIn={setUser} onError={(m) => showToast('error', m)} />
  ...
  return (
    <div className="app">
      <Header state={state} user={user} onLogout={handleLogout} />
      <nav className="tabs">
        {[
          ['practice', '練習'],
          ['formation', '編成'],
...
function Header({ state, user, onLogout }) {
  return (
    <header className="header">
      ...
      <div className="account">
        ...
        <button className="logout" onClick={onLogout}>ログアウト</button>
      </div>
    </header>
  )
}
...
// ===================== 認証(ログイン/新規登録) =====================
function AuthScreen({ onLoggedIn, onError }) {
  ...
  return (
    <div className="onboarding auth-screen">
      <h1>Pedal Nations</h1>
      <p className="lead">
        エフェクターを擬人化した収集×放置育成ゲーム。{mode === 'login' ? 'ログイン' : '新規登録'}してはじめよう。
      </p>
      ...
      <form className="auth-form" onSubmit={submit}>
        ...
        {mode === 'register' && <p className="muted small">パスワードは8文字以上。</p>}
      ...
      <button className="link" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setLocalError(null) }}>
